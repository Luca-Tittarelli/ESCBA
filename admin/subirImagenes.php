...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_FILES)) {
	...
	if ($check !== false){
		...
		$statement = $conexion->prepare('INSERT INTO imagenes (id, noticias_id, epigrafe, imagen, borrado) 
														VALUES (null, :id_noticia, :epigrafe, :imagen, 0)');
		...
		header('Location: subirImagenes.php?id='.$id_noticia.'&e='.$mensaje);
	} else {
		$errores.= "El archivo no es una imagen o el archivo es muy pesado";
		header("Location: " . RUTA . 'admin/subirImagenes.php?id='.$id_noticia.'&errores='.$errores); //volvemos al editarUsuario 
